...
const days: [Weekday, string][] = [[1, 'Monday'], [2, 'Tuesday'], [3, 'Wednesday'], [4, 'Thursday'], [5, 'Friday'], [6, 'Saturday']];
...
function renumberTimetable(timetable: TimetablePeriod[]): TimetablePeriod[] {
  const nextSequence = new Map<Weekday, number>();
  return timetable.map((period) => {
    const sequence = (nextSequence.get(period.weekday) ?? 0) + 1;
    nextSequence.set(period.weekday, sequence);
...
export function ConfigEditor({ initialConfig, sections, initialSectionId, initialSectionName, configsBySection, updatedAtBySection, calendarUpdatedAtByYear }: { initialConfig: ScheduleConfig; sections: SectionOption[]; initialSectionId: string; initialSectionName: string; configsBySection: Record<string, ScheduleConfig>; updatedAtBySection: Record<string, string | null>; calendarUpdatedAtByYear: Record<number, string> }) {
  ...
  useEffect(() => {
    ...
    router.refresh();
  }, [state.success, state.sectionId, activeSectionId, router]);
// Switching sections resets the editable draft to that section's saved
  ...
  const removePeriod = (index: number) => { const remaining = config.timetable.filter((_, periodIndex) => periodIndex !== index); update({ timetable: renumberTimetable(remaining) }); };
  const movePeriod = (fromIndex: number, toIndex: number) => { if (toIndex < 0 || toIndex >= config.timetable.length || config.timetable[toIndex]?.weekday !== config.timetable[fromIndex]?.weekday) return; const reordered = [...config.timetable]; const [period] = reordered.splice(fromIndex, 1); reordered.splice(toIndex, 0, period); update({ timetable: renumberTimetable(reordered) }); };
  ...
  const removeHoliday = (index: number) => update({ holidays: config.holidays.filter((_, itemIndex) => itemIndex !== index) });
    const addSaturday = () => {
      ...
      const used = new Set(config.specialSaturdays.map((special) => special.date));
      while (date.toISOString().slice(0, 10) <= config.semesterEnd && used.has(date.toISOString().slice(0, 10))) date.setUTCDate(date.getUTCDate() + 7);
      const saturday = date.toISOString().slice(0, 10);
      if (saturday > config.semesterEnd) return;

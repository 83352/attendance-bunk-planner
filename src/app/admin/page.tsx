...
export default async function AdminPage({ searchParams }: AdminPageProps) {
  ...
  return <main className="admin-shell"><p className="eyebrow">Admin configuration</p><h1>Schedule control room</h1><p>Authenticated as {user.email}. Keep the timetable current and the public calculation follows it.</p><ConfigEditor key={selectedSection?.id ?? 'new'} initialConfig={loaded.config} updatedAt={loaded.updatedAt} sections={availableSections} selectedSectionId={selectedSection?.id ?? ''} initialSectionName={selectedSection?.name ?? 'CSE 5'} /></main>;
}

...
function ChipRow({ sections, activeId, onSelect }: { sections: SectionOption[]; activeId: string; onSelect: (id: string) => void }) {
  // Roving tabindex keyboard nav. Mirrors SectionSelector.tsx so the two
  // chip groups behave identically for keyboard users.
  const initialIndex = Math.max(0, sections.findIndex((s) => s.id === activeId));

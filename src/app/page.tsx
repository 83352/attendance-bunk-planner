...
async function loadPublicConfig(sectionId?: string): Promise<{ config: ScheduleConfig; sectionName: string; sections: { id: string; name: string }[]; selectedSectionId: string }> {
  ...
  return { sectionName: section.name, sections, selectedSectionId: section.id, config: { semesterStart: semester.start_date, semesterEnd: semester.end_date, timetable: savedTimetable, holidays: (holidays ?? []).map((item) => ({ name: item.name, start: item.start_date, end: item.end_date })), specialSaturdays: (specialSaturdays ?? []).map((item) => ({ date: item.date, copiedWeekday: item.copied_weekday })), exams: (exams ?? []).map((item) => ({ name: item.name, start: item.start_date, end: item.end_date, periodsPerDay: item.periods_per_day, dailyPeriods: (examDays ?? []).filter((day) => day.exam_name === item.name).map((day) => ({ date: day.date, periodsPerDay: day.periods_per_day })) })) } };
}
